...
export async function validateAdminAccess(request: Request, env: Env): Promise<{ valid: boolean; error?: string }> {
...
export function createDBRoutes() {
  ...
  app.get('/users', async (c) => {
    ...
    if (!env.D1_DATABASE) return errorResponse('Database unavailable', 503);
  ...
  app.post('/users', async (c) => {
    ...
    try {
      ...
      if (emailExists.length > 0) return errorResponse("Email already exists", 409);
      const usernameExists = await db.select().from(users).where(eq(users.username, createData.username)).limit(1);
      if (usernameExists.length > 0) return errorResponse("Username already exists", 409);
  ...
  app.put('/users/:id', async (c) => {
    ...
    try {
      ...
      if (updatedUser.length === 0 || !updatedUser[0]) return errorResponse("Failed to retrieve updated user", 500);
      return jsonResponse({ success: true, data: updatedUser[0] });
    } catch (error) {
      if (error instanceof ZodError) {
        return jsonResponse({ error: 'Invalid request data', details: error.errors }, 400);
      }
  ...
  app.delete('/users/:id', async (c) => {
    ...
    return jsonResponse({ success: true, message: "User suspended and all API keys revoked" });
  });
  ...
  app.post('/api-keys', async (c) => {
    ...
    try {
      ...
      if (userExists.length === 0) return errorResponse("User not found", 404);
  ...
  app.put('/api-keys/:id', async (c) => {
    ...
    try {
      ...
      if (existingKey.length === 0) return errorResponse("API key not found", 404);
      const updateFields: any = { updatedAt: new Date() };
      if (updateData.name) updateFields.name = updateData.name;
      ...
      if (updatedKey.length === 0 || !updatedKey[0]) return errorResponse("Failed to retrieve updated API key", 500);
  ...
  app.all('/*', async (c) => errorResponse('Not implemented', 501));
