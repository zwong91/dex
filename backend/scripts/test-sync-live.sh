...
echo "  http://localhost:8787/v1/api/dex/sync/trigger"
echo "  http://localhost:8787/v1/api/dex/pools/discover"
echo ""
echo "启动开发服务器..."
...
# 测试基本端点
echo "测试基本端点..."
curl -s -H "$API_HEADER" http://localhost:8787/v1/api/dex/status | jq '.' || echo "端点响应错误"

echo ""
echo "🔍 触发池发现..."
curl -s -H "$API_HEADER" -X POST http://localhost:8787/v1/api/dex/pools/discover | jq '.' || echo "池发现请求失败"

echo ""
echo "🔄 触发同步..."
curl -s -H "$API_HEADER" -X POST http://localhost:8787/v1/api/dex/sync/trigger | jq '.' || echo "同步请求失败"
